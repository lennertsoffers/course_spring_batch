...
public class JobConfiguration {
    ...
    public Flow searchFlow() throws Exception {
        ...
        return new FlowBuilder<SimpleFlow>("searchFlow")
                .start(providerDecider)
                .on("DUBAI_AMSTERDAM_OFFER_ONLY")
                .to(this.dubaiAmsterdamOffer())
                .from(providerDecider)
                .on("DEFAULT_SEARCH")
                .to(this.searchAirlineFlow())
                .from(providerDecider)
                .on("NEW_YORK_AMSTERDAM_OFFER_INCLUDED")
                .to(this.newYorkAmsterdamOffer())
                .next(this.searchAirlineFlow())
                .build();
    }

    public Flow searchAirlineFlow() throws Exception {
        Flow flow1 = airlineFlow("adios", this.requestAdiosStep());
        Flow flow2 = airlineFlow("flyUs", this.requestFlyUsStep());
        Flow flow3 = airlineFlow("sp", this.requestSouthPacificStep());
        Flow flow4 = airlineFlow("ta", this.requestTransamericanStep());
        Flow flow5 = airlineFlow("be", this.requestBelarusStep());
        Flow flow6 = airlineFlow("oc", this.requestOceanicStep());
    ...
    private Flow airlineFlow(String flowName, Step step) throws Exception {
        SimpleFlow simpleFlow = new FlowBuilder<SimpleFlow>(flowName)
                .start(step)
                .build();
